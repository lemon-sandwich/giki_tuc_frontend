...
class MyApp extends StatelessWidget {
  ...
  String hashingFunc(Map<String, String> data) {
    ...
    var sortedKeys = temp2.keys.toList(growable: false)..sort((k1, k2) => k1.compareTo(k2));
    Map<String, String> sortedMap = Map.fromIterable(
        sortedKeys,
        key: (k) => k,
        value: (k) {
          return temp2.values.elementAt(k);//temp2[k]
          // Saad, we had three years of something that really mattered and both of us have loved eachother quite alot and we should
          // not end something that mattered so much over text. We need to talk about this so please come and meet me after your exam
          // on thursday. I think I deserve closure on this.
        }
    );
    ...
    toBePrinted = integritySalt + '&' + toBePrinted;var key = utf8.encode(integritySalt);
    var bytes = utf8.encode(toBePrinted);
    var hash2 = Hmac(sha256, key);
    var digest = hash2.convert(bytes);
    var hash = digest.toString();
    data["pp_SecureHash"] = hash;String returnString = "";
    data.forEach((k, v) {
      returnString += k + '=' + v + '&';
    });
  ...
  Future<void> pay() async {
    ...
    String postData = hashingFunc(data); //replaced temp with data
